...
{ 
	LCD_RS_CLR;//写地址  
 	LCD_CS_CLR; 
	GPIO_WriteOutData(GPIOD,data);//DATAOUT(data); 
	LCD_WR_CLR; 
	LCD_WR_SET; 
 	LCD_CS_SET;   
}
...
{										   
	u16 t;
/*****************************设置输出模式******************************************************/	
	GPIO_PullResistorConfig(GPIOD,GPIO_PIN_ALL, GPIO_PR_UP);//设置上拉模式
	GPIO_DirectionConfig(GPIOD,GPIO_PIN_ALL,GPIO_DIR_IN);//设置IO方向
	GPIO_InputConfig(GPIOD,GPIO_PIN_ALL,ENABLE);//设置输入模式 
	GPIO_SetOutBits(GPIOD,GPIO_PIN_ALL);//置高
/*************************************************************************************/
	...
	LCD_CS_SET; 
/*****************************设置输出模式***************************************************/		
	GPIO_InputConfig(GPIOD,GPIO_PIN_ALL,DISABLE);//失能输入模式
	GPIO_DriveConfig(GPIOD,GPIO_PIN_ALL, GPIO_DV_8MA);//设置引脚驱动电流
  GPIO_PullResistorConfig(GPIOD,GPIO_PIN_ALL, GPIO_PR_UP);//设置浮空模式
  GPIO_DirectionConfig(GPIOD,GPIO_PIN_ALL,GPIO_DIR_OUT);//设置IO方向	
	GPIO_WriteOutData(GPIOD,0XFFFF);
/*************************************************************************************/
	return t;  
}
//写寄存器
...
{
 	u16 r,g=0,b=0;//May error possiblely
...
void LCD5310_init (void)
{
	delay_ms(50); // delay 50 ms 
	LCD_WriteReg(0x0000,0x0001);
	delay_ms(50); // delay 50 ms 	
	lcddev.id = LCD_ReadReg(0x0000);
 	if(lcddev.id==0||lcddev.id==0XFFFF)//读到ID不正确
	{
				LCD_WR_REG(0XD4);				   
...
void lCD9431_init (void)
{
 	...
 	if(lcddev.id==0||lcddev.id==0XFFFF)//读到ID不正确
	{
		...
		LCD_RD_DATA(); 			//dummy read 	
 		LCD_RD_DATA();   	    //读回0X00
  	lcddev.id=LCD_RD_DATA()&0XBF;//这里读回0XD3,实际是0X93才对.强制去掉第6位   								   
 		lcddev.id<<=8;
...
void LCD_Init(void)
{  
  CKCU_APBPerip0ClockConfig(CKCU_APBEN0_PC | CKCU_APBEN0_AFIO, ENABLE);//使能APB0外围时钟
	
	AFIO_GPCConfig(AFIO_PIN_9|AFIO_PIN_11|AFIO_PIN_12|AFIO_PIN_13|AFIO_PIN_14,AFIO_MODE_DEFAULT);//配置GPIOA引脚复用模式
	
	GPIO_DriveConfig(GPIOC, GPIO_PIN_9|GPIO_PIN_11|GPIO_PIN_14|GPIO_PIN_13|GPIO_PIN_12, GPIO_DV_8MA);//设置引脚驱动电流
  GPIO_PullResistorConfig(GPIOC, GPIO_PIN_9|GPIO_PIN_11|GPIO_PIN_14|GPIO_PIN_13|GPIO_PIN_12, GPIO_PR_DISABLE);//设置浮空模式
  GPIO_DirectionConfig(GPIOC, GPIO_PIN_9|GPIO_PIN_11|GPIO_PIN_14|GPIO_PIN_13|GPIO_PIN_12,GPIO_DIR_OUT);//设置IO方向
	GPIO_SetOutBits(GPIOC,GPIO_PIN_9|GPIO_PIN_11|GPIO_PIN_14|GPIO_PIN_13|GPIO_PIN_12);//置高
	
	
	CKCU_APBPerip0ClockConfig(CKCU_APBEN0_PD| CKCU_APBEN0_AFIO, ENABLE);//使能APB0外围时钟
	AFIO_GPDConfig(AFIO_PIN_ALL,AFIO_MODE_DEFAULT);//配置GPIOD引脚复用模式
	GPIO_DriveConfig(GPIOD,GPIO_PIN_ALL, GPIO_DV_8MA);//设置引脚驱动电流
  GPIO_PullResistorConfig(GPIOD,GPIO_PIN_ALL, GPIO_PR_DISABLE);//设置浮空模式
  GPIO_DirectionConfig(GPIOD,GPIO_PIN_ALL,GPIO_DIR_OUT);//设置IO方向
	GPIO_SetOutBits(GPIOD,GPIO_PIN_ALL);//置高
